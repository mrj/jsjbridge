...
browser.storage.local.get(['licensed', 'licensedVersion', 'lastLicenceCheckTimeMS']).then(storedValues => {
  ...
  browser.runtime.onConnect.addListener(portToContentScript => {
      ...
      if (messageFromContentScript.type == 'newApplet') {
        ...
        if (appletClassesForProgram && appletClassesForProgram.includes(appletClass)) {
          ...
          if (portToNative) {
            ...
            portToNative = nativePortsByProgramName[programName] = browser.runtime.connectNative(programName);
